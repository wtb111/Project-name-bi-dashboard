...
def load_service_account_credentials(path: str):
    if google_service_account is None or GoogleRequest is None:
        raise RuntimeError(
            "Missing google-auth dependency. Install with: pip install google-auth"
        )
    creds = google_service_account.Credentials.from_service_account_file(path, scopes=GA4_SCOPE)
    ...
    if mode == "cohort-active-users" and retention_rows:
        for row in retention_rows:
            ...
            if not first_date or not active_date:
                continue
            try:
                # 与 KNIME 的 Date&Time Difference 保持一致：Second minus first
                # 即 firstSessionDate - date，Row Filter 过滤 -1 / -3 / -7
                diff = (datetime.strptime(first_date, "%Y-%m-%d") - datetime.strptime(active_date, "%Y-%m-%d")).days
            except ValueError:
                continue
            label = {-1: "次留", -3: "3留", -7: "7留"}.get(diff)
            if not label:
                continue
            # Joiner 按 date = date，因此留存值归属到 active_date
            if active_date not in lookup:
                lookup[active_date] = {"次留": 0, "3留": 0, "7留": 0}
            lookup[active_date][label] += int(round(row.get("activeUsers", 0)))
        return lookup
...
def build_output_rows(raw_rows: List[Dict], retention_mode: str = "zero", retention_rows: List[Dict] | None = None) -> List[Dict]:
    ...
    for row in raw_rows:
        ...
        ga_new_users = int(round(row.get("newUsers", 0)))
        # 与 KNIME Math Formula 对齐：老用户日活 = activeUsers - newUsers
        new_users = ga_new_users
        old_active = max(active - ga_new_users, 0)
...
def main():
    ...
    retention_start_date = (datetime.strptime(start_date, "%Y-%m-%d") - timedelta(days=45)).strftime("%Y-%m-%d")
    retention_body = {
